#Note these points, we may have to do some changes
#inference_engine with flask server is not able to give some top-left corner boxes coordinates
#inference_engine without flask server is able to give some top-left corner boxes coordinates as well as gave the correct box count 
#File Handlingfor the output directory is required. Done
#Mission_Folder timestamp should be changed.Done
#Relative Path also needs to be included in place of absolute paths.Done
...
import pandas as pd
import json
import inference_engine_noserver
import datetime

# import sys
# Data-Capture->Pre-processing->Box-DetectionDL->Box_tvecsS->Box_count->Json_compile->vision_report_sender

def master_script(input_main_dir,task_id):
    parser = argparse.ArgumentParser(description='Description of your program')
    parser.add_argument('-d', '--debug', help='Description for Debug argument',
                        default='Normal')
    parser.add_argument('-b', '--barcode', help='Description for barcode function',
                        default='Normal')
    ...
    #barcode_method = args['barcode']
    barcode_method = "Dynamosoft"
    # Get current time stamp
    time_stamp = str(time.time())
        # directory1 is an intermediate variable for getting outer_folder_name
        # i.e Mission Folder
    directory1 = input_main_dir
    ...
    output_dir = directory1
    #Add the task_id to Mission folder name along with timestamp
    if not os.path.exists("../output"):
        output_dir = os.mkdir("../output")
    tmstmp = str(datetime.datetime.now())

    outer_folder_name = (directory1 + "/" + "Mission_" + "task_id" + str(task_id) + "_" + tmstmp)
    ...
    station_names = glob.glob(input_main_dir)
    #print(station_names)
    for input_dir in station_names:
        input_image_list = []
        input_image_barcode_list = []
        ...
        for img in images:
            if "barcode" in img:
                input_image_barcode_list.append(img)
            else:
        ...
        for img in good_imglist:
            ...
            print("Image name: ", img_name)
            corners4, counter =  \
                inference_engine_noserver.Box_Coordinates(img, img_name, depthImage,
                                                     file_box_corners,
                                                     file_box_centre,
        ...
        for i in rows:
            ...
            st = " "
        # print("rows: ",rows)
        # Aggregate images pallet wiseStocktakingU
        index = 0
        for val in rows[:]:
...
#      input_dir = '/home/nishant/Wipro/work/Dataset/data_test/*'
#      station_names = glob.glob(input_dir)
#      #print(station_names)
#      for station in station_names:
#      #master_script('/home/nishant/Wipro/work/Dataset/data_test/fov_data5',2)
